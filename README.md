...
- `HEALTH_URL`：健康检查地址，默认 `http://127.0.0.1:18790/ready`（可用环境变量覆盖，如非默认端口或远程探测；旧部署可覆盖回 `/health`）。默认用 `/ready`（readiness：至少一个账号 healthy 且不在 cooldown 才 200）而非 `/health`（liveness：进程起来就 200，无法发现“所有账号不可用”的部署）。`/ready` 是 **fail-closed 部署闸门**：当所有账号都在 cooldown/exhausted（如上游密钥失效、上游故障）时健康验证失败、部署自动回滚——这是设计意图，新版本把账号状态搞坏时绝不该上线；只想要 liveness 级检查时显式设 `HEALTH_URL=http://127.0.0.1:18790/health` 覆盖。
- `HEALTH_TIMEOUT`：健康等待窗口（秒），默认 `35`，必须为正整数（非法值在任何副作用发生前 `exit 3`，prism 不受影响）。restart 后每 1 秒轮询 `systemctl is-active` + `HEALTH_URL`，成功立即继续，窗口耗尽才回退；回退后的健康验证使用同一轮询。默认值覆盖 prism 启动时的首轮账号探测窗口（单账号最长 `ProbeTimeout=30s`，HTTP server 在该探测完成后才开始监听）。**不要**把 `HEALTH_TIMEOUT` 盲目延长到数分钟来“等”全账号恢复——那只会把一次注定失败的部署拖到很晚才回滚；全账号不可用应直接回滚并排查（`journalctl -u prism`）。
...
| Field | Description |
|-------|-------------|
| `name` | Label; also builds `LB_KEY_*` name. MUST be non-empty and unique, start with an ASCII letter/digit, and contain only ASCII letters/digits/`_`/`-` (max 64 bytes); two names that fold to the same `LB_KEY_*` (e.g. `a-b` vs `a_b`) are rejected at load |
