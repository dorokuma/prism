...
func TestStripCodexUpstreamBloat_Truncation(t *testing.T) {
	// Build a string with config.SystemPromptMaxRunes + 100 runes
	over := config.SystemPromptMaxRunes + 100
	...
	// Should be truncated to config.SystemPromptMaxRunes + truncation suffix
	if !strings.HasSuffix(got, config.TruncationSuffix) {
		t.Fatalf("expected truncation suffix, got (last 100 chars): %q", got[len(got)-min(100, len(got)):])
	}

	gotRunes := []rune(got)
	suffixRunes := utf8.RuneCountInString(config.TruncationSuffix)
	expectedTotal := config.SystemPromptMaxRunes + suffixRunes
	if len(gotRunes) != expectedTotal {
		t.Errorf("truncated length = %d runes, want %d runes (%d content + %d suffix)",
			len(gotRunes), expectedTotal, config.SystemPromptMaxRunes, suffixRunes)
	}

	// First config.SystemPromptMaxRunes should be 'a' characters
	for i := 0; i < config.SystemPromptMaxRunes; i++ {
		if gotRunes[i] != 'a' {
...
func TestStripCodexUpstreamBloat_ExactlyAtLimit(t *testing.T) {
	// Build a string exactly at config.SystemPromptMaxRunes
	var sb strings.Builder
	for i := 0; i < config.SystemPromptMaxRunes; i++ {
		sb.WriteRune('x')
	}
	...
	if got != exactInput {
		t.Errorf("string at limit should not be truncated, got length %d, want %d", len([]rune(got)), config.SystemPromptMaxRunes)
	}
}
...
func TestStripCodexUpstreamBloat_MultiByteUTF8(t *testing.T) {
	...
	var sb strings.Builder
	for i := 0; i < config.SystemPromptMaxRunes+10; i++ {
		sb.WriteString("世")
	}
	input := sb.String()
	...
	// Should have config.SystemPromptMaxRunes content runes + suffix
	gotRunes := []rune(got)
	suffixRunes := utf8.RuneCountInString(config.TruncationSuffix)
	if len(gotRunes) != config.SystemPromptMaxRunes+suffixRunes {
		t.Errorf("multi-byte truncated length = %d runes, want %d", len(gotRunes), config.SystemPromptMaxRunes+suffixRunes)
	}
...
func TestStripCodexUpstreamBloat_DebugLogging(t *testing.T) {
	...
	var sb strings.Builder
	for i := 0; i < config.SystemPromptMaxRunes+100; i++ {
		sb.WriteRune('a')
	}
...
func TestStripCodexUpstreamBloat_RemovesAndTruncates(t *testing.T) {
	...
	// still exceeds config.SystemPromptMaxRunes → removal first, then truncation.
	var sb strings.Builder
	// Fill prefix so it alone exceeds the limit
	for i := 0; i < config.SystemPromptMaxRunes+500; i++ {
		sb.WriteRune('a')
	}
	sb.WriteString("\n<skills_instructions>\n")
	...
	gotRunes := []rune(got)
	suffixLen := utf8.RuneCountInString(config.TruncationSuffix)
	if len(gotRunes) != config.SystemPromptMaxRunes+suffixLen {
		t.Errorf("after removal+truncation got %d runes, want %d (content) + %d (suffix) = %d",
			len(gotRunes), config.SystemPromptMaxRunes, suffixLen, config.SystemPromptMaxRunes+suffixLen)
	}
}
